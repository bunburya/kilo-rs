...
impl Terminal {
    ...
    /// Read a single byte from the terminal and return it
    pub(crate) fn read_byte(&self) -> Result<u8> {
        let mut buf = [0u8];
    ...
    /// Print an escape sequence to move the cursor to the given row and column. Row and column are 1-based.
